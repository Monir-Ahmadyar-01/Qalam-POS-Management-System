                        ...
                        <div class="card-box table-responsive">
                            <table id="datatable-buttons" class="table table-striped table-bordered  nowrap">
                                ...
                                <tbody>
                                    <?php
                                        $count = 1;
                                        $sql_query_001 = mysqli_query($connection,"select `qalam_mis`.`suppliers`.`id` AS `id`,`qalam_mis`.`suppliers`.`full_name` AS `full_name`,`qalam_mis`.`suppliers`.`phone_number` AS `phone_number`,`qalam_mis`.`suppliers`.`address` AS `address`,`qalam_mis`.`suppliers`.`date` AS `date`,sum(`qalam_mis`.`purchase_minor`.`purchase_price` * `qalam_mis`.`purchase_minor`.`amount`) AS `total_purchase_price`,(select sum(`qalam_mis`.`purchase_major`.`reciept`) from `qalam_mis`.`purchase_major` where `qalam_mis`.`purchase_major`.`supplier_id` = `qalam_mis`.`suppliers`.`id`) AS `total_reciept` from ((`qalam_mis`.`suppliers` left join `qalam_mis`.`purchase_major` on(`qalam_mis`.`suppliers`.`id` = `qalam_mis`.`purchase_major`.`supplier_id`)) left join `qalam_mis`.`purchase_minor` on(`qalam_mis`.`purchase_minor`.`purchase_major_id` = `qalam_mis`.`purchase_major`.`id`)) group by `qalam_mis`.`suppliers`.`id`");
